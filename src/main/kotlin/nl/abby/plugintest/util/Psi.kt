 ...
 */
fun PsiElement.inComment() = inDirectEnvironmentContext(Context.COMMENT) || when (this) {
    is PsiComment -> true
...
fun LatexBeginCommand.environmentName(): String? =
    beginOrEndEnvironmentName(this)

/**
 * Finds the [LatexEndCommand] that matches the begin command.
 */
fun LatexBeginCommand.endCommand(): LatexEndCommand? = nextSiblingOfType(
    LatexEndCommand::class)
 ...
 */
fun LatexEndCommand.beginCommand(): LatexBeginCommand? = previousSiblingOfType(
    LatexBeginCommand::class)

/**
 * Checks if the latex content objects is a display math environment.
 */
fun LatexContent.isDisplayMath() = firstChildOfType(LatexDisplayMath::class) != null && firstChildOfType(
    LatexEnvironment::class) == null
